...
exports.run = async (client, message, args) => {
  ...
  if (acod.includes(args[0])) {
        ...
        return message.channel.send(
          aredembed.setDescription(
            ":x: CapsLock Engelleme sitemi zaten açık bulunmakta!\nBir mesajdaki büyük harf oranı eğer mesajın `" +
              acikmi.yuzde +
              "` sinden daha fazla ise engelenecektir.\nEğerki bu oranı arttırmak veya azaltmak isterseniz `m!capslock aç oran` (Oran 101 den küçük, 0 dan büyük bir sayı olmalıdır!)\nEğer CapsLock Engelleme sistemini kapatmak isterseniz `m!capslock sıfırla` komutunu kullanabilirsiniz."
          )
        );
    ...
    if (!acikmi)
      return message.channel.send(
        aredembed.setDescription(
          "CapsLock Engelleme sistemi zaten kapalı.\nEğer açmak isterseniz `m!capslock aç oran` yazabilirsiniz. (Oran 101 den küçük, 0 dan büyük bir sayı olmalıdır!)\nNot: Eğer oran yazmazsanız otomatik olarak 50% olarak ayarlanacaktır."
        )
      );
    message.channel.send(
      new Discord.MessageEmbed()
        ...
        .setAuthor("CapsLock Engelleme")
        .setDescription(
          "CapsLock Engelleme sistemi başarıyla kapatıldı!\nArtık mesajlardaki büyük harfler engellenmeyecek. \nEğer tekrar açmak isterseniz `m!capslock aç oran` komutunu kullanabilirsiniz. (Oran 101 den küçük, 0 dan büyük bir sayı olmalıdır!)\nNot: Eğer oran yazmazsanız otomatik olarak 50% olarak ayarlanacaktır."
        )
        .setFooter(
          `Komut ${message.author.username} tarafından kullanıldı.`,
          message.author.displayAvatarURL({ dynamic: true })
    ...
    let acikkk;
    if (acikmi)
      acikkk = `${acikmi.yuzde}% olarak Açık**\nEğer kapatmak isterseniz \`m!capslock sıfırla\` yazabilirsiniz.`;
    let kodare = new Discord.MessageEmbed()
      ...
      .setDescription(
        "CapsLock Engelleme sistemi şu anda **" +
          (acikkk
            ? acikkk
            : "Kapalı**\nEğer açmak isterseniz `m!capslock aç` yazabilirsiniz.") +
          "."
      )
      .setFooter(
        `Komut ${message.author.username} tarafından kullanıldı.`,
...
exports.help = {
  ...
  usage: "capslock aç/sıfırla oran (Oran 101 den küçük, 0 dan büyük bir sayı olmalıdır!)"
};
