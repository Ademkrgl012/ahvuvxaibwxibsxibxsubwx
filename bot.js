  ...
  if (anahtar === "acik") {
    ...
    if (küfürler.some(küfür => msg.content.toLowerCase().includes(küfür))) {
      if (!msg.member.hasPermission("ADMINISTRATOR")) {
        msg.delete().then(msg.reply("Bu Sunucuda Küfür Etmene İzim Veremiyeceğim!"));
      }
    }
  }
  if (!anahtar) return;
...
client.on("guildMemberRemove", async member => {
  ...
  var randomMsg = ["Güle Güle Sensiz Yolumuza Devam Edeceğiz😪"];
  var randomMsg_integer =
    randomMsg[Math.floor(Math.random() * randomMsg.length)];
...
client.on("guildMemberAdd", async member => {
  ...
  var randomMsg = ["Sunucumuza Hoşgeldin Seni Burada Gördüğüme Sevindim🎉"];
  var randomMsg_integer =
    randomMsg[Math.floor(Math.random() * randomMsg.length)];
...
const voiceChannels = message.guild.channels.cache.filter(c => c.type === 'voice');
    ...
    if(message.content.toLowerCase() === '.say') {
        ...
        .addField("Boşta üye Sayısı",message.guild.members.cache.filter(m => !m.user.bot && m.user.presence.status == "idle").size)
